    ...
    <nav className="w-full border-b border-gray-200 bg-white/70 backdrop-blur-md sticky top-0 z-30">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
        ...
        <a href="#/" className="flex items-center gap-2">
          ...
          <span className="font-bold text-xl tracking-tight text-[#122944]">
            Komanda Ryšys
          </span>
        </a>
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {/* NEW: Subscriptions */}
          <NavLink href="#/subscriptions" label="Subscriptions" className="text-[#1662A6] font-semibold" />
          ...
          {user ? (
            <>
              ...
              <button
                onClick={logout}
                className="text-red-600 text-sm ml-4 hover:text-red-800"
              >
                Logout
              </button>
            </>
        ...
        <div className="md:hidden border-t border-gray-200 bg-white p-4 space-y-4">
          ...
          {user ? (
            <div className="space-y-2">
              ...
              >
                <div className="h-9 w-9 flex items-center justify-center rounded-full bg-[#122944] text-white text-sm font-semibold">
                  {initial}
                </div>
              ...
              <button
                onClick={logout}
                className="text-red-600 text-sm underline px-1"
              >
                Logout
              </button>
            </div>
            ...
            <a
              href="#/auth"
              className="block rounded-xl bg-[#1662A6] text-center py-2 text-white font-semibold"
            >
              Login
            </a>
          )}
...
function NavLink({ href, label }) {
  return (
    <a
      href={href}
      className="text-sm font-medium text-gray-700 hover:text-[#122944] transition"
    >
      {label}
    </a>
  );
...
function MobileLink({ href, label }) {
  return (
    <a
      href={href}
      className="block py-2 text-gray-800 text-sm font-medium"
    >
      {label}
    </a>
  );
